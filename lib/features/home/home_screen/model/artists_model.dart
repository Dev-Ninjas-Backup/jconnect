...
class HighlightMedia {
  ...
  const HighlightMedia({required this.url, required this.isVideo});

  factory HighlightMedia.fromJson(dynamic item) {
    if (item is Map<String, dynamic>) {
      final url = item['fileLink']?.toString() ?? item['url']?.toString() ?? '';
      final explicitType =
          item['type']?.toString().toLowerCase() ??
          item['mediaType']?.toString().toLowerCase() ??
          item['fileType']?.toString().toLowerCase() ??
          '';
      return HighlightMedia(
        url: url,
        isVideo: explicitType.contains('video') || _isVideoUrl(url),
      );
    }

    final url = item?.toString() ?? '';
    return HighlightMedia(url: url, isVideo: _isVideoUrl(url));
  }
